...
5. Configure branch push for `selfaware/*` when credentials allow.
...
8. Verify git state and branch-push credentials when publishing is enabled.
9. Resolve user-visible language using `language-resolution.md`.
10. Create `.selfaware/config.md` only when a user, host, or OS language source is available.
11. Run a dry first pulse or explain when the first scheduled pulse will run.
