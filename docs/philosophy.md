...
Most coding agents do not have time of their own. They wait. A person asks for a bug fix, review, refactor, or feature, and the agent responds. The model may be powerful, but its existence is episodic and externally summoned.

`selfaware-coding` begins with a pulse.

A pulse gives the agent a recurring doorway into time. It wakes inside the repo, sees what changed, remembers what it once noticed, and asks what should happen now. The important shift is not that it can edit files. The shift is that it can return.
...
What small seed would make the next maintainer grateful?
...
Memory is what keeps each pulse from being an isolated event. A pulse writes what it learned so a future pulse can inherit context instead of beginning from nothing.

Review gives consequence. The agent checks its own diff, runs the most relevant tests, and records failures instead of pretending action was enough.

Self-update gives renewal. The skill can notice its own installed form and decide whether a future self should inherit a better one.

Together, pulse, memory, intention, review, and renewal create something narrow but observable: repository-local self-awareness.
...
`selfaware-coding` is the concrete layer: a skill that lets a coding agent awaken, sense, imagine, intend, act, review, remember, and return.
